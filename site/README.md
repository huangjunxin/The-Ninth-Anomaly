...
正文按界面语言分流：`en` 界面读**英文原稿**（`books/.../chapters/`），
`yue` 界面读**粤文译本**（`books/.../yue/chapters/`），`ja` 界面读
**日译本**（`books/.../ja/chapters/`）；i18n 同时作用于界面文案。

## 页面与路由

| 路由 | 说明 |
| --- | --- |
| `/` · `/yue/` · `/ja/` | 首页：CSS 氛围 hero + 章节目录（含每章阅读时长；英文按 ~230 词/分钟、粤文/日文按 ~500 字/分钟估算） |
| `/chapters/chapter-NN/` · `/yue/chapters/chapter-NN/` · `/ja/chapters/chapter-NN/` | 章节阅读页（1–16）：左侧章节侧栏、阅读进度条、上一章/下一章导航、亮暗双主题 |

- `en` 为默认语言且不加前缀（`/`、`/chapters/...`）；粤文界面位于 `/yue/`，日语界面位于 `/ja/`。
- 共 51 个静态页面：英 17 + 粤 17 + 日 17（各首页 + 16 章）。
...
- 所有界面文案集中在 `src/i18n/ui.ts` 的字典（`ui.en` / `ui.yue` / `ui.ja`），组件通过
  `t(locale, key, vars?)` 取文案，无散落硬编码。书名 "The Ninth Anomaly" 三
  种界面都保留英文主标题；粤文界面额外展示副标题「第九異常」及粤文版
  logline，日语界面额外展示副标题「第九の異常」及日文版 logline。
- **语言自动识别**：`BaseLayout` 的 `<head>` 内联脚本在首屏渲染前执行——用户
  若从未手动选择过语言（localStorage `tna-lang` 无值），按 `navigator.language`
  判断：`zh-*` 浏览器跳转到 `/yue/`、`ja-*` 跳转到 `/ja/` 对应页面，其余留在
  `/`；跳转会保留当前路径（章节页对章节页）。用户一旦点击页眉
  「EN / 粵文 / 日本語」切换按钮，`tna-lang` 被写入，此后始终尊重手动选择、
  不再自动跳转。
- 语言切换按钮位于页眉（主题按钮左侧），三个语言各自链接到当前页面的对应
  版本（`/chapters/chapter-01/` ↔ `/yue/chapters/chapter-01/` ↔
...
└── src/
    ├── content.config.ts   # Content Layer：glob 读取 ../books/...（chapters 英文原稿 + chaptersYue 粤文译本 + chaptersJa 日译本）
    ├── i18n/ui.ts          # 界面文案字典（en/yue/ja）+ t() 取值与路径助手
    ├── lib/
    │   ├── book.ts                 # 章节解析（英/粤/日标题格式）/ 阅读时长 / 排序辅助
    ...
    ├── pages/
    │   ├── index.astro             # en 首页（/）
    │   ├── chapters/[slug].astro   # en 章节页（/chapters/...，16 个静态路由）
    │   ├── yue/index.astro         # yue 首页（/yue/）
    │   ├── yue/chapters/[slug].astro# yue 章节页（/yue/chapters/...，粤文译本）
...
# 期望：51 个页面（en / + 16 章，yue /yue/ + 16 章，ja /ja/ + 16 章），无 /full
```